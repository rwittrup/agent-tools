...
Compose **Graphite CLI steps** with **PR description formatting** from the skills below.

## Read first

1. **Graphite mechanics** — Read the **graphite** skill for `gt create`, `gt modify`, `gt submit`, `gt ls`, `gt restack`, branch naming, untracked branches, stack parentage, troubleshooting.
2. **PR body shape** — Read the **pr-formatting** skill for the full template (Summary, Context, Changes, Acceptance), rules, and frontend-only Validation variant.

Do not invent a different PR outline; use the **pr-formatting** template and rules.

## End-to-end flow

1. **Branch and commits** — Follow **graphite**: stage changes, `gt create … -m "…"` (or `gt modify`), correct stack parent (`main`), `gt ls` to confirm.
...
3. **Submit** — `gt submit --no-interactive` (**graphite**).
4. **Titles and bodies (required)** — GitHub/pr-lint expects allowed title formats per root `AGENTS.md` (e.g. `[TICKET-123] description`, `[HF]`, `[BUMP]`, `Bump`). After PRs exist:
   - **Never** use Bash heredocs for markdown bodies (escaping breaks content). Write the body to a file (e.g. `/tmp/pr-body.md`) using your harness's file-write capability, then:
...
- Stacked PR rationale and splitting: **graphite** (“What Makes a Good PR?”)
